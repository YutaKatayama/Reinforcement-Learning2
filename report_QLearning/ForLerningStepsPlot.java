                        ...
                        executor.submit(() -> {
                            ...
                            try {
                                ...
                                try (PrintWriter writer = new PrintWriter(
                                    ...
                                    // 開始位置とゴール位置の出力
                                    writer.printf("-------------------------------------------------------------------------------%n");
                                    writer.printf("        開始位置(1,1)%n");
                                    writer.printf("        ゴール位置(8,8)%n");
                                    ...
                                    while ((line = reader.readLine()) != null) {
                                        ...
                                        if (line.contains("最短ステップ数:")) {
                                            Pattern pattern = Pattern.compile("最短ステップ数:(\\d+)");
                                            Matcher matcher = pattern.matcher(line);
                                            if (matcher.find()) {
                                                int shortestSteps = Integer.parseInt(matcher.group(1));
                                                writer.printf("        最短ステップ数:%d%n", shortestSteps);
                                            }
                                        }
                                        ...
                                        if (line.contains("学習回数")) {
                                            ...
                                            if (matcher.find()) {
                                                ...
                                                // 各学習回数とステップ数をターミナルに出力
                                                System.out.printf("学習回数：%d ゴールまでのステップ数:%d これまでの最小ステップ数:%s%n",
                                                    learningCount, steps, "---");
                                                // CSVにも記録
                                                writer.printf("学習回数：%d ゴールまでのステップ数:%d これまでの最小ステップ数:%s%n",
                                                    learningCount, steps, "---");
                                            }
                                        }
                                    }
                                    ...
                                    if (foundOptimal) {
                                        writer.printf("**********%n最適ルートを獲得**********%n");
                                        writer.printf("学習回数：%d ゴールまでのステップ数:%d%n", 
                                            Integer.parseInt(line.split(" ")[1]), Integer.parseInt(line.split(" ")[3]));
                                    }

                                    // パラメータの出力
                                    writer.printf("-------------------------------------------------------------------------------%n");
                                    writer.printf("パラメータ%n");
                                    writer.printf("ε=%.2f, α=%.2f, γ=%.2f%n", e, a, g);
                                    System.out.printf("使用したパラメータ: ε=%.2f, α=%.2f, γ=%.2f%n", e, a, g);
                                }
